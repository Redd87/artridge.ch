...
function displayAchievementIcons() {
  ...
  for (let i = 0; i < achievements.length; i++) {
    ...
    const showDescription = a.value >= a.levels[0].n || i === 0 && refactorLevel > 0;
    if (i >= 3 && i <= 12 && window.buildata && a.value >= a.levels[0].n) {
      div.innerHTML += `<img style="width: 100%; height: 100%;" src="images/Projects/${buildata[i-3].name}/finished.png">`
    } else {
      div.innerHTML = `
        <span ${showDescription ? '' : 'class="unknown-achievement"'}>${showDescription ? a.description : "?"}</span>
      `;
    }
...
function openAchievement(n) {
  ...
  for (let i=0; i<a.levels.length; i++) {
    let image = ''
    if (i === 0 && n >= 3 && n <= 12) {
      image = `background: url(images/Projects/${buildata[n-3].name}/finished.png);
    ...
    checkpoints += `<div style="top: ${topPos};${image}" class="${maxedOut ? "maxed-out" : ''} check ${i === 0 ? "first" : l.unlocked ? (l.permanent ? "unlocked-permanent" : "unlocked-notpermanent") : (l.permanent ? "notunlocked-permanent" : "notunlocked-notpermanent")}">
...
function init(saveData) {
  ...
  for (let i = 0; i<specialBuildings.length; i++) {
    buildata[specialBuildings[i]].specialData.locked = false;
  }
